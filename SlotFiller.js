...
module.exports = {
			...
			if (queryIndex[i].index == -1) {
				...
				for (let j = i + 1; j < queryIndex.length; j++) {
					...
					if (keywordIndex == -1) {//It's not a keyword so group it.
						...
						} else {//otherwise it's just one word 'tuna'
							break;
						}
					} else {
						break;
	...
	fixForNeighboringWildcards(wildCardIndex, reducedIndex) {
		...
		for (let i = 0; i < wildCardIndex.length - 1; i++) {
			debug('inside loop', i, wildCardIndex[i].index, wildCardIndex[i + 1].index)
	...
	closestWildcardMatch(wildCardIndex, reducedIndex) {
		...
		for (let i = 0; i < minLength; i++) {
			wildcard[wildCardIndex[i].wcIndex] = reducedIndex[indexes[i + bestShift]].word;
		}
		...
		return {wildcards : wildcard, score : { score : wcScore, count : (totalCount-missCount)}};
	},
